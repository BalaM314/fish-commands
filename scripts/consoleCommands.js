...
exports.commands = (0, commands_1.consoleCommandList)({
    ...
    status: {
        ...
        handler: function (_a) {
            ...
            output("\nStatus:\n\tPlaying on map &fi".concat(Vars.state.map.plainName(), "&fr\n\t").concat(Vars.state.rules.waves ? "Wave &b".concat(Vars.state.wave, "&fr, &b").concat(Math.ceil(Vars.state.wavetime / 60), "&fr seconds until next wave.\n") : "", "\t&b").concat(Groups.unit.size(), "&fr units, &b").concat(Vars.state.enemies, "&fr enemies, &b").concat(Groups.build.size(), "&fr buildings\n\tTPS: ").concat((0, utils_1.colorNumber)(Core.graphics.getFramesPerSecond(), function (f) { return f > 58 ? "&g" : f > 30 ? "&y" : f > 10 ? "&r" : "&br&w"; }, "server"), ", Memory: &b").concat(Math.round(Core.app.getJavaHeap() / 1048576), "&fr MB\n\tServer uptime: ").concat((0, utils_1.formatTime)(uptime), " (since ").concat((0, utils_1.formatTimestamp)(Date.now() - uptime), ")\n").concat([
                globals_1.fishState.restarting ? "&lrRestart queued&fr" : "",
                players_1.FishPlayer.antiBotMode() ? "&br&wANTIBOT ACTIVE!&fr" + (0, utils_1.getAntiBotInfo)("server") : "",
            ].filter(function (l) { return l.length > 0; }).join("\n"), "\n").concat((0, utils_1.colorNumber)(Groups.player.size(), function (n) { return n > 0 ? "&b" : "&lr"; }, "server"), " players online, ").concat((0, utils_1.colorNumber)(numStaff, function (n) { return n > 0 ? "&b" : "&lr"; }, "server"), " staff members.\n").concat(players_1.FishPlayer.mapPlayers(function (p) {
                return "\t".concat(p.rank.shortPrefix, " &b").concat(p.uuid, "&fr &c").concat(p.name, "&fr");
